...
public class LuaJavaClassExposer {
   ...
   @LuaMethod(name="fullApiAccess", global=true)
   ...
   private void loadClasses(java.io.File directory, String packageName) {
	   ...
	   if (packageName.contains("org.lwjglx.opengl") ||
		   packageName.contains("org.junit") ||
		   packageName.contains("astar.tests") ||
		   packageName.contains("zombie.iso") ||
		   packageName.contains("zombie.core.opengl")) {
		   zombie.debug.DebugLog.Lua.println("[API Unlocker] [DEBUG] Ignored package " + packageName);
		 return;
	   }
	   ...
	   for (java.io.File file : files) {
		   ...
		   if (name.contains("test") ||
			   name.contains("Test")  ||  
			   name.contains("KahluaConverterManager") ||
			   name.contains("LuaCompiler") ||
			   name.contains("ActionContext") ||
			   name.contains("PacketTypes") ||
			   name.contains("CustomPerks") ||
			   name.contains("ItemPickerJava") ||
			   name.contains("GameWindow") ||
			   name.contains("ServerGUI") ||
			   name.contains("MainScreenState") ||
			   name.contains("ModelManager") ||
			   name.contains("SpawnRegions")) {
			 zombie.debug.DebugLog.Lua.println("[API Unlocker] [DEBUG] Ignored class " + name);
			 continue;  
		   }
   ...
   public void exposeGlobalFunctions(Object var1) {
	  if (!loaded) {
		  loaded = true;
		  zombie.debug.DebugLog.Lua.println(
				  "[API Unlocker] Loading all classes in folder '"
				+ new File("java").getAbsolutePath()
			    + "' for full game api access.");
		  this.exposeGlobalFunctions(this);
		  this.loadClasses(new File("java"), "");
		  zombie.debug.DebugLog.Lua.println("[API Unlocker] Loaded " + this.environment.size() + " classes");
	  }
   ...
   public void exposeLikeJava(Class var1, KahluaTable var2) {
      if (var1 != null && !this.isExposed(var1)) {
   ...
   private void exposeLikeJava(KahluaTable var1, Set<Type> var2, Type var3) {
      if (var3 != null) {
         if (!var2.contains(var3)) {
        	zombie.debug.DebugLog.Lua.println("[API Unlocker] [DEBUG] {exposeLikeJava} Exposing " + var3.toString());
            var2.add(var3);
            if (var3 instanceof Class) {
               if (!this.shouldExpose((Class)var3)) {
            	   zombie.debug.DebugLog.Lua.println("[API Unlocker] [DEBUG] {exposeLikeJava} " + var3.toString() + " will be not exposed");
                  return;
               }
   ...
   private void exposeList(KahluaTable var1, Set<Type> var2, Type[] var3) {
      Type[] var4 = var3;
   ...
   private void exposeLikeJavaByClass(KahluaTable var1, Set<Type> var2, Class<?> var3) {
